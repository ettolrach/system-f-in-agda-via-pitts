...
We aim to prove that our definition of local closure is equivalent to the definition that Pitts TODO
CITE gives.

\begin{code}
  open import Data.Product using (∃; ∃-syntax)
  _≻'_ : ℕ → Type → Set
  i ≻' A = (j : ℕ) ⦃ _ : j ≥ i ⦄ → ∃[ T ] (ty-ty[ j —→ T ] A ≡ A)
\end{code}
...
The proof that the cofinite definition implies the existential one is simple. The cofinite
quantification gives a `disallow' list. Simply choose an identifier not in that list, and use
this identifier for the existential quantification.
\begin{code}
  ≻ty⇒≻' i A i≻tyA j =
    let И⟨ Иe₁ , Иe₂ ⟩ = i≻tyA j
...
Proving the other implication is much more difficult. To do this we need to use an alternative
definition of local closure given by TODO: CITE CHARGUÉRAUD.
\begin{code}
  data _lc-at_ : Type → ℕ → Set where
    lc-at-‵ℕ : ∀ {i}
      → ‵ℕ lc-at i
    ...
    lc-at-⇒ : ∀ {i A B} → A lc-at i → B lc-at i
      → (A ⇒ B) lc-at i

    lc-at-∀ᵀ : ∀ {i A} → A lc-at (suc i)
      → (∀ᵀ A) lc-at i
\end{code}

Now we prove the implication by induction on the structure of the type.
\begin{code}
  ≻'⇒≻ty i ‵ℕ i≻'A = n≻‵ℕ
  ≻'⇒≻ty i (frᵀ X) i≻'A j = И⟨ [] , (λ _ → refl) ⟩
  ≻'⇒≻ty i (#ᵀ k) i≻'A j with j ≟ℕ k
  ... | no  j≢k  = И⟨ [] , (λ _ → refl) ⟩
  ... | yes refl with i≻'A j
