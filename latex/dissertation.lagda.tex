...
\textit{identifiers} (usually these are strings). An alternative representation uses \textit{de Bruijn
indices}, where a variable is referred to using a natural number representing how many binders ago
the variable was bound, first described by Nicolaas Govert \citet{de_bruijn_lambda_1972}. By using
identifiers for free variables and de Bruijn indices for bound variables, we use a syntax called the
\textit{locally nameless representation}, first used by Andrew \citet{goos_mechanisation_1994}. More
detail on this syntax is given in section \ref{background:locally_nameless}.

Most of the prior literature uses the proof-assistant Coq for its formalisation of the locally
...
this work did not focus on the $\lambda$-calculus, it demonstrated that the syntax is able to be
used effectively in Agda. As such, I would like to explore formalising System F in Agda using
locally nameless representation using the methods shown in the work presented by Pitts. His work was
essential in helping develop crucial initial proofs of the syntax and informed future proof
strategies.
...
these points. A more detailed evaluaton of my formalisation as well as future work will be presented
in chapter \ref{chapter4}.

To do so, I will follow the work of Arthur \citet{chargueraud_locally_2012}, where he describes a
formalisation of System F with subtyping in Coq. Although I will closely follow him initially, from
...
exception where a regular proof has been provided, see page \pageref{chapter3:subst}). An in-depth guide to Agda is
presented in \citet{wadler_programming_2022}. Knowledge of Agda (or proof-assistants which are
similar) will be assumed henceforth.
...
As mentioned before, \citet{pitts_locally_2023} explored a generalisation called \textit{locally
nameless sets} in Agda. Since the scope of his work was only to prove properties of the syntax and
not of any lanugage in particular, the helpfulness of his development is limited. By relying on only
the opening and closing operations, one cannot manipulate language constructs enough to prove
properties such as \texttt{subst-open}, as explained by \citet{kiselyov_generic_2025}. Although they
present a library which does prove properties such as \texttt{subst-open}, their work is written in
Coq and so cannot be used here.
...
formalising System F, nor is it necessaary to understand this formalisation. The only functions from
...
dissertation, I will leave some proofs from this chapter and present them in appendix
\ref{appendix:f} instead. I will only do this in two cases:
\begin{itemize}
...
of the other local closure properties.
\begin{code}
...
$i$, then $A$ is locally closed up to $i + 1$. The property does not follow from Lemma 2.6 as that
would prove $i \succ \forall A$ implies $i + 1 \succ \forall A$ without removing the $\forall$.
\begin{code}
...
have no effect on the term. This is, in fact, true for combinations of type-opening too, so we need
...
The proofs of the related versions of this lemma are left in appendix \ref{appendix:f_local_closure}
...
substituted for is not present in the type. While this is also true for the other forms of
...
the map is applied. The proof is a simple induction on $\Gamma$ and so left in appendix
\ref{appendix:f_contexts}.
\begin{code}
...
\begin{itemize}
  ...
  variables to the context, so we should not add any bound type variables either.
...
\paragraph*{Accessing variables in the context.} Accessing the context is done in two different ways:
\begin{itemize}
...
We want to apply an equality in a type judgement, so we define helper functions as we did for type
accessors.
\begin{code}
...
that is not in the previously defined list.
...
\end{code}
The case for $\vdash\lambdabar$ is very similar, except that we only do not need to append any other
list to \texttt{Иe₁} in our own cofinite quantification.
\begin{code}
...
We want to swap variables at the top of the context and drop shadowed ones. Since these are also a
corollary of the \texttt{rename} lemma, their proof is left in appendix
...
trivial induction induction on the structure of the term. Note that this is for a
\textit{term-in-term} substitution.
\begin{code}
...
swap between \texttt{:=} with opening and \texttt{:→}. The proof is a trivial induction, and so
...
Agda's normalise feature. For example, in Emacs, one can press \texttt{C-c} \texttt{C-n} and type
\texttt{eval (gas 10) ⊢twice-suc-zero} to see the derivation being automatically generated.
...
However, to do so, we required proofs which were very difficult to explain. It is unclear if a
different proof approach would have given a simpler function body. Additionally, despite following
Agda's requirements for termination, the compiler still complains that \texttt{subst} does not
terminate. Of course, termination-checking is undecidable, but a proof of \texttt{subst} without the
\texttt{\{\#- TERMINATING -\#\}} pragma would be desirable.
...
the locally nameless representation requires infrastructure for the syntax to be defined. Since
bound variables outside of a $\lambda$-abstraction are ill-typed, it would prove difficult to
...
self-representation. Thus, an Agda development would not be unreasonable to develop.
...
\citet{pitts_locally_2023} gives.
...
Specifically, we will prove an if and only if relation between the two. We will only prove this for
the type-in-type local closure, but the proof for the other two openings works similarly.
\begin{code}
  ≻ty⇒≻'ty : ∀ (i : ℕ) (A : Type)
    → i ≻ty A → i ≻'ty A
...
The first proof is straight-forward. Since we can always create a new identifier, we simply create
one that is not in the `disallow' list provided by the cofinite quantifier.
...
Now we prove the other implication by induction on the structure of the type.

The base cases \texttt{‵ℕ}, and \texttt{frᵀ} are trivial. For a bound variable, if the bound
variable is not the same as the index, we don't replace and so the proof is completed by
reflexivity. If they are the same, then that means that the other local closure would have opened
the term, which would \textit{not} have left the term unchanged---a contradiction. The other cases
are the inductive steps and simply use the inductive hypothesis together with the injectivity of
\texttt{⇒}, \texttt{∀ᵀ}, and \texttt{‵suc}.
\begin{code}
  ≻'ty⇒≻ty i ‵ℕ i≻'tyA = n≻‵ℕ
