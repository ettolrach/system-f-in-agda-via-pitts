...
We also have some local closure properties which will become useful in later proofs.
\begin{code}
  n≻‵ℕ : ∀ {n} → n ≻ ‵ℕ
  n≻‵ℕ j = И⟨ [] , (λ _ → refl) ⟩
...
One important lemma which will be used below is called \texttt{open-rec-lc-lemma} by
...
several versions of the proof. We cannot use a generic proof because it is proven by induction on
the structure of the term or type.
\begin{code}
  -- This is true too, but included in the appendix
  -- as we don't need it.
...
We'll need to collect free variables for some later theorems.
\begin{code}
...
This substitution is central to proving evaluation properties, so we'll require a few proofs of how
it interacts with local closure.
\begin{code}
  :=-≻ : ∀ {A x C i j}
    → j ≥ i
    → i ≻ty A
...
Contexts are isomorphic to lists \citep{wadler_programming_2022}, so we can also concatenate them
and define map over the types of free variables. We will need this later in proofs.
...
added to the context (or in other words, \textit{in scope}). Since the STLC has no free type
variables, there wasn't a need for this predicate. We closely follow how
\citet{chargueraud_locally_2012} defines it.

Before we can define the predicate, however, we need to be able to reason about the domain of a
context. We're only interested in the free type variables that are contained in a context and those
contained in the types of the free variables in the context (\texttt{domain-all-ftv}).
\begin{code}
...
Now we can prove that these two contexts are `Ok'.
\begin{code}
  _ : Ok (((∅ , ('T' ∷ [])) , ('Q' ∷ [])) , ('x' ∷ []) ⦂ (t-fr ('T' ∷ [])))
...
Finally, we have two more convenience functions which we'll use later.
\begin{code}
  ok-+ : ∀ {Γ Δ} → Ok (Γ + Δ) → Ok Γ
  ok-+ {Γ} {∅} okΓ = okΓ
  ...
  extract-⊆ : ∀ {Γ x A}
    → Ok (Γ , x ⦂ A) → ftv-ty A ⊆ domain-ftv Γ
...
We have some more functions for reasoning about what the contents of a context. We want to convert
between evidence that a variable is in the context and evidence that a variable is in the domain of
the context.
\begin{code}
...
Finally, we also want to remove a free type variable from the context by applying a substitution,
and importantly, that this gives us the new type of a free variable. These ideas are critical to
proving how substitution is preserved when evaluating.
\begin{code}
  ∋-map-ftv : ∀ {Γ X x A C}
    (Δ : Context)
    → Ok ((Γ , X) + Δ)
  ...
  ftv⊆dom-:= {‵ℕ} Γ Δ lc-C C⊆Δ okΓ+map A⊆dom = All.[]
  ftv⊆dom-:= {X} {t-fr Y} {C} Γ Δ lc-C C⊆Δ okΓ+map (Y∈ All.∷ All.[])
    with X ≟lchar Y
  ... | yes refl = All-map
...
has these type judgements:
\begin{equation}
\begin{gathered}
  \inferrule
    { }
    {\text{Ok} \Gamma\\\Gamma \vdash \texttt{‵zero} \colon \nat}
...
(besides the substyping) is that we only allow a type-application for types without free variables.
\begin{code}
  infix  4 _⊢_⦂_
...
We first would like to show that all well-typed terms have an `Ok' environment.
\begin{code}
  ⊢⇒Ok : ∀ {Γ L A} → Γ ⊢ L ⦂ A → Ok Γ
...
The proofs of the remaining properties is similar to those in \citet{wadler_programming_2022}.
\begin{code}
  -- Extending contexts.
  ext-tm : ∀ {Γ Δ}
    → (∀ {x A}     →        Γ ∋ x ⦂ A → Δ ∋ x ⦂ A)
...
We have a few remaining properties.
\begin{code}
  -- Weakening contexts.
  weaken : ∀ {Γ L A}
    → Ok Γ
...
allow us to replace the \texttt{:→} substitution with the \texttt{:=} substitution later.
\begin{code}
...
\begin{code}
  ...
  ⊢⇒lc {Γ} (⊢Λ И⟨ Иe₁ , Иe₂ ⟩) j =
    И⟨ Иe₁ , (λ a {a∉} →
      cong Λ_ (lemma2·7-2 ⦃ LnsTerm ⦄ z≤n (helper z≤n (⊢⇒lc (Иe₂ a {a∉}))))) ⟩
    where
      helper : ∀ {N i k q x}
        → k ≥ i
        → i ≻tm (ty-tm[ q —→ x ] N)
      ...
      helper {ƛ N} {i} {k} {q} {x} k≥i i≻ty j =
        let И⟨ Иe₁ , Иe₂ ⟩ =
          (helper {k = suc i} (≤-refl) (≻ƛ⇒s≻ƛ i≻ty))
            (suc j)
            ⦃ s≤s (≤-trans k≥i it) ⦄
        in И⟨ Иe₁ , (λ a {a∉} → cong ƛ_ (Иe₂ a {a∉})) ⟩
...
We also need to one kind of substitution out of another.
\begin{code}
...
Finally, we can prove that substitution preserves types. \texttt{subst-ty} shows that we can
substitute a free type variable and have the type of the term effected accordingly, and
\texttt{subst} shows that we can apply a substitution to eliminate a function type of a term.
\begin{code}
...
The proof of progress is quite straight-forward.
\begin{code}
...
The proof of preservation, however, proves difficult. It requires the difficult proofs of substitution which were presented earlier.
\begin{code}
  preserve : ∀ {L L' A}
    → ∅ ⊢ L ⦂ A
    → L —→ L'
...
\chapter{Conclusions}
We have successfully developed an evaluator for System F.
