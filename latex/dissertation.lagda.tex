...
\begin{code}
  ...
  -- ty-ctx[_:=_]_ : List Char → Type → Context → Context
  -- ty-ctx[ X := A ] ∅ = ∅
  -- ty-ctx[ X := A ] (Γ , y ⦂ B) = (ty-ctx[ X := A ] Γ) , y ⦂ (ty-ty[ X := A ] B)
  -- ty-ctx[ X := A ] (Γ , Y) = ty-ctx[ X := A ] Γ

  -- ok-ctx-:= : ∀ {Γ X A} → Ok Γ → Ty-LocallyClosed A → ¬ (ftv-ty A ⊆ domain-ftv Γ) → Ok (ty-ctx[ X := A ] Γ)
  -- ok-ctx-:= {∅} ok-∅ _ _ = ok-∅
  -- ok-ctx-:= {Γ , y ⦂ B} (ok-∷fv okΓ lc-B B⊆Γ) lc-A A⊈Γ = ok-∷fv (ok-ctx-:= okΓ lc-A A⊈Γ) (:=-≻ B z≤n lc-B lc-A) {!!}
  -- ok-ctx-:= {Γ , Y} okΓ lc-A A⊈Γ = {!!}
\end{code}
...
--   subst-ty {X = Y} ⊢L = {!!}
  ...
  subst-ty {X = Y} {A = A} {B = B ⇒ C} lc-A (⊢free (ok-∷ftv okΓ X∉Γ) lc-B⇒C (T⦂ {x = x} ∋x)) =
    ⊢free okΓ (:=⇒-≻ B C z≤n lc-B⇒C lc-A) (⇒-:= B C (≡-with-∋ ∋x (sym (:=-∉-idempotent (∉-domain-all-∋ X∉Γ ∋x)))))
  subst-ty {X = Y} {B = t-∀ B} lc-A (⊢free (ok-∷ftv okΓ X∉Γ) lc-B (T⦂ ∋x)) =
    ⊢free okΓ (:=∀-≻ B z≤n lc-B lc-A) (∀-:= B (≡-with-∋ ∋x (sym (:=-∉-idempotent (∉-domain-all-∋ X∉Γ ∋x)))))
--   subst-ty {X = Y} {B = B} lc-A (⊢free okΓ,Y lc-B ∋x) = ⊢free {!!} (:=-≻ B z≤n lc-B lc-A) {!!}
  ...
  subst-op-ty {L} {B} {C} lc-C (⊢Λ И⟨ Иe₁ , Иe₂ ⟩) =
    ...
    in ≡-with-⊢-ty (≡-with-⊢-tm (subst-ty lc-C (Иe₂ x {x∉Иe₁})) (sym (subst-intro-ty-tm L x∉ftv-L))) (sym (subst-intro-ty-ty B x∉ftv-B))
\end{code}
...
\begin{code}
  ...
  preserve (⊢[] lc-A ⊢L) (β-Λ 1≻L _) = {!!} -- subst-op-ty x ⊢L
