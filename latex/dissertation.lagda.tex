...
module chapter4 where
  open import cofinite
  open import plfa_adaptions using (∉∷[]⇒≢; ∉-++; ∉y∷ys⇒≢y; m+1≤n⇒m≤n)
  open import tspl_prior_work
...
\begin{code}
  ...
  ...   | yes refl = refl
  ...   | no  i≢k  = refl
  tm-tm-swap-ty-tm {ƛ A , L} {i} {j} {x} {y} =
    cong (ƛ ty-ty[ j —→ y ] A ,_) (tm-tm-swap-ty-tm {L})
...
\begin{code}
  ...
  open-rec-lc-lemma-ty-tm : ∀ {L i j u v}
    ...
    → ty-tm[ i —→ u ] L ≡ L
  open-rec-lc-lemma-ty-tm {fr x} assump = refl
  open-rec-lc-lemma-ty-tm {# k} assump = refl
  open-rec-lc-lemma-ty-tm {ƛ A , L} assump rewrite
      proj₁ (ƛ-inj assump)
    | open-rec-lc-lemma-ty-tm {L} (proj₂ (ƛ-inj assump))
    = refl
  open-rec-lc-lemma-ty-tm {L · M} assump rewrite
      open-rec-lc-lemma-ty-tm {L} (proj₁ (·-inj assump))
    | open-rec-lc-lemma-ty-tm {M} (proj₂ (·-inj assump))
    = refl
  open-rec-lc-lemma-ty-tm {Λ L} assump rewrite
    open-rec-lc-lemma-ty-tm {L} (Λ-inj assump) = refl
  open-rec-lc-lemma-ty-tm {L [ A ]} assump rewrite
      proj₂ ([]-inj assump)
    | open-rec-lc-lemma-ty-tm {L} (proj₁ ([]-inj assump))
    = refl
  open-rec-lc-lemma-ty-tm {‵zero} assump = refl
  open-rec-lc-lemma-ty-tm {‵suc L} assump rewrite
    open-rec-lc-lemma-ty-tm {L} (‵suc-inj assump) = refl
\end{code}
...
\begin{code}
  ...
  ax1-ty-tm i a b (ƛ A , L)
    rewrite ax1-type i a b A | ax1-ty-tm i a b L = refl
  ax1-ty-tm i a b (L · M)
    rewrite ax1-ty-tm i a b L | ax1-ty-tm i a b M = refl
  ax1-ty-tm i a b (Λ L)
...
\begin{code}
  ...
  A⇒A-lc : ∀ {A} → Ty-LocallyClosed (t-fr A ⇒ t-fr A)
  A⇒A-lc j = И⟨ [] , (λ _ → refl) ⟩
  ⇒-≻ : ∀ {A B i} → i ≻ (A ⇒ B) → (i ≻ A) × (i ≻ B)
  ⇒-≻ {A} {B} {i} i≻A⇒B = ⟨ i≻A , i≻B ⟩
    where
  ...
  i≻ty-tmƛ : ∀ {L A i} → i ≻ty-tm (ƛ A , L) → (i ≻ty A) × (i ≻ty-tm L)
  i≻ty-tmƛ {L} {A} {i} i≻ = ⟨ i≻A , i≻L ⟩
    where
      i≻A : i ≻ty A
      i≻A j = let И⟨ Иe₁ , Иe₂ ⟩ = i≻ j
  ...
  module example where
    twice : ∅ ⊢ (Λ ƛ ((t-# 0) ⇒ (t-# 0))
      , (ƛ (t-# 0) , ((# 1) · ((# 1) · (# 0)))))
        ⦂ t-∀ (((t-# 0) ⇒ (t-# 0))
          ⇒ ((t-# 0) ⇒ (t-# 0)))
    twice = ⊢Λ И⟨ [] , (λ A {A∉} → ⊢ƛ A⇒A-lc И⟨ [] , (λ f {f∉} →
      ⊢ƛ ty-fr-lc И⟨ (f ∷ []) , (λ x {x∉} → ⊢·
        (⊢free A⇒A-lc (T (f≢x x∉) H′))
        (⊢· (⊢free A⇒A-lc (T (f≢x x∉) H′)) (⊢free ty-fr-lc H′))) ⟩) ⟩) ⟩
      where
        f≢x : ∀ {f x} → x ∉ f ∷ [] → f ≢ x
        f≢x x∉ = sym-≢ (∉∷[]⇒≢ x∉)
    ...
    big-omega : ∅ ⊢ (ƛ (t-∀ ((t-# 0) ⇒ (t-# 0)))
      , (((# 0) [ t-∀ ((t-# 0) ⇒ (t-# 0)) ]) · (# 0)))
        ⦂ (t-∀ ((t-# 0) ⇒ (t-# 0)))
          ⇒ (t-∀ ((t-# 0) ⇒ (t-# 0)))
    big-omega =
      ⊢ƛ lc-forall И⟨ [] , (λ x {x∉} → ⊢· (⊢[] (⊢free lc-forall H′) lc-forall) (⊢free lc-forall H′)) ⟩
        where
...
\begin{code}
  ...
  rename : ∀ {Γ Δ}
    → (∀ {x A} → Γ ∋ x ⦂ A → Δ ∋ x ⦂ A)
      ---------------------------------
    → (∀ {L A} → Γ ⊢ L ⦂ A → Δ ⊢ L ⦂ A)
  rename ρ (⊢free lc-A x) = ⊢free lc-A (ρ x)
  rename {Δ = Δ} ρ (⊢ƛ lc-A И⟨ Иe₁ , Иe₂ ⟩) = ⊢ƛ lc-A
    И⟨ (domain Δ ++ Иe₁)
...
\begin{code}
  ...
  -- tm-tm-swap-ty-tm-ctx : ∀ {Γ A L i j x y}
  --   → Γ ⊢ (tm-tm[ i —→ x ] (ty-tm[ j —→ y ] L)) ⦂ A
  --   → Γ ⊢ (ty-tm[ j —→ y ] (tm-tm[ i —→ x ] L)) ⦂ A
  -- tm-tm-swap-ty-tm-ctx {L = L} assump = ≡-with-⊢ assump (tm-tm-swap-ty-tm {L})
\end{code}
...
proof of this in System F is presented below. TODO: Talk.
\begin{code}

  s≻ƛ⇒≻ƛ : ∀ {L A i} → (suc i) ≻tm L → i ≻tm (ƛ A , L)
  s≻ƛ⇒≻ƛ {A = A} s≻ j = let И⟨ Иe₁ , Иe₂ ⟩ = s≻ (suc j) ⦃ s≤s it ⦄
    in И⟨ Иe₁ , (λ a {a∉} → cong (ƛ A ,_) (Иe₂ a {a∉})) ⟩

  ≻ƛ⇒s≻ƛ : ∀ {L A i} → i ≻tm (ƛ A , L) → (suc i) ≻tm L
  ...
  ⊢⇒lc (⊢free x lc-A) j = И⟨ [] , (λ a → refl) ⟩
  ⊢⇒lc (⊢ƛ lc-A И⟨ Иe₁ , Иe₂ ⟩) j = И⟨ Иe₁ , (λ a {a∉} → cong₂ ƛ_,_ refl
    (open-rec-lc-lemma
      (λ ())
  ...
  ⊢⇒lc {Γ} (⊢Λ И⟨ Иe₁ , Иe₂ ⟩) j = И⟨ domain Γ ++ Иe₁ , (λ a {a∉} → cong Λ_ (lemma2·7-2 ⦃ LnsTerm ⦄ z≤n (helper z≤n (⊢⇒lc (Иe₂ a {proj₂ (∉-++ a∉)}))))) ⟩
    where
      helper : ∀ {N i k q x}
        → k ≥ i
        → i ≻tm (ty-tm[ q —→ x ] N)
  ...
  ⊢⇒lc-ty {Γ} {fr x} (⊢free lc-A ∋x) = lc-A
  ⊢⇒lc-ty {Γ} {ƛ B , L} (⊢ƛ lc-B И⟨ Иe₁ , Иe₂ ⟩) j =
  ...
  ⊢⇒lc-ty {Γ} {L [ B ]} (⊢[] ⊢L lc-B) =
    ...
    where
      ...
      helper {t-∀ A} {B} {i} j≥i si≻∀A i≻B k =
        ...
        in И⟨ Иe₁ , (λ a {a∉} → cong t-∀ (Иe₂ a {a∉})) ⟩
  ⊢⇒lc-ty {Γ} {‵zero} ⊢zero = n≻‵ℕ
  ⊢⇒lc-ty {Γ} {‵suc L} (⊢suc ⊢L) = n≻‵ℕ

  ⊢⇒lc-ty-tm : ∀ {Γ L A} → Γ ⊢ L ⦂ A → Ty-Tm-LocallyClosed L
  ⊢⇒lc-ty-tm (⊢free x lc-A) j = И⟨ [] , (λ _ → refl) ⟩
  ⊢⇒lc-ty-tm (⊢ƛ lc-A И⟨ B-Иe₁ , B-Иe₂ ⟩) j =
    ...
    in И⟨ Иe₁ ++ A-Иe₁ , (λ a {a∉} →
      cong₂ ƛ_,_
        (A-Иe₂ a {proj₂ (∉-++ {xs = Иe₁} a∉)})
        (open-rec-lc-lemma-ty-tm (Иe₂ a {proj₁ (∉-++ a∉)}))) ⟩
  ⊢⇒lc-ty-tm {Γ} (⊢· ⊢L ⊢M) j = И⟨ domain Γ , (λ a {a∉} → cong₂ _·_
    (lemma2·7-2 ⦃ LnsTyTm ⦄ it (⊢⇒lc-ty-tm ⊢L))
    (lemma2·7-2 ⦃ LnsTyTm ⦄ it (⊢⇒lc-ty-tm ⊢M))) ⟩
  ⊢⇒lc-ty-tm {Γ} {Λ L} (⊢Λ И⟨ Иe₁ , Иe₂ ⟩) j = И⟨ Иe₁ , (λ a {a∉} → cong Λ_ {!!}) ⟩
  ⊢⇒lc-ty-tm {Γ} (⊢[] ⊢L lc) j = И⟨ domain Γ , (λ a → cong₂ _[_]
    (lemma2·7-2 ⦃ LnsTyTm ⦄ it (⊢⇒lc-ty-tm ⊢L))
    (lemma2·7-2 ⦃ LnsType ⦄ it lc)) ⟩
  ⊢⇒lc-ty-tm ⊢zero j = И⟨ [] , (λ _ → refl) ⟩
  ⊢⇒lc-ty-tm {Γ} (⊢suc ⊢L) j = И⟨ domain Γ , (λ a →
...
\begin{code}
  ...
  subst-open-ty-tm (ƛ A , L) x≢y j≥i i≻N
  ...
  subst-open-ty-tm (‵suc L) x≢y j≥i i≻N =
    subst-open-ty-tm L x≢y j≥i i≻N
  ...
  subst : ∀ {Γ x L N A B}
    ...
    → Γ , x ⦂ A ⊢ L ⦂ B
      --------------------
    → Γ ⊢ tm-tm[ x := N ] L ⦂ B
  subst {x = y} ⊢N (⊢free lc-A (H refl)) with y ≟lchar y
  ... | yes refl = weaken ⊢N
  ... | no  y≢y  = contradiction refl y≢y
  subst {x = y} ⊢N (⊢free {x = x} lc-A (T x≢y ∋x)) with y ≟lchar x
  ... | yes refl = contradiction refl x≢y
  ... | no  _    = ⊢free lc-A ∋x
  subst {x = y} {L = ƛ _ , L} ⊢N (⊢ƛ lc-A И⟨ Иe₁ , Иe₂ ⟩) =
    ⊢ƛ lc-A И⟨ (y ∷ Иe₁) , (λ a {a∉} → 
      let a∉Иe₁ = proj₂ (∉-++ {xs = y ∷ []} a∉)
          a≢y = (∉y∷ys⇒≢y a∉)
          ⊢tm-tm[]L = swap a≢y (Иe₂ a {a∉Иe₁})
      in subst-open-var-ctx L (sym-≢ a≢y) (⊢⇒lc ⊢N) (subst ⊢N ⊢tm-tm[]L)) ⟩
  subst {x = y} ⊢N (⊢· ⊢L ⊢M) = ⊢· (subst ⊢N ⊢L) (subst ⊢N ⊢M)
  -- subst-open-var-ctx : ∀ {Γ A} {N : Term} {x y : List Char} {i : ℕ}
  --   (L : Term)
  --   → x ≢ y
  --   → (i ≻ N)
  ...
  subst {x = y} {L = Λ L} ⊢N (⊢Λ И⟨ Иe₁ , Иe₂ ⟩) =
    ⊢Λ И⟨ y ∷ Иe₁ , (λ a {a∉} →
      let a∉Иe₁ = proj₂ (∉-++ {xs = y ∷ []} a∉)
          y≢a = sym-≢ (∉y∷ys⇒≢y a∉)
          ⊢ty-tm[]L = Иe₂ a {a∉Иe₁}
      in subst-open-ty-tm-ctx L y≢a z≤n (⊢⇒lc-ty-tm ⊢N) (subst ⊢N ⊢ty-tm[]L) ) ⟩
  subst {x = y} ⊢N (⊢[] ⊢L lc) = ⊢[] (subst ⊢N ⊢L) lc
  subst {x = y} ⊢N ⊢zero = ⊢zero
  subst {x = y} ⊢N (⊢suc ⊢L) = subst ⊢N ⊢L
\end{code}
