...
identifiers (usually strings). An alternative representation uses de Bruijn indices, where a
variable is referred to using a natural number representing how many binders ago the variable was
bound, first described by Nicolaas Govert \citet{de_bruijn_lambda_1972}. By using identifiers for
free variables and de Bruijn indices for bound variables, we use a syntax called the \textit{locally
nameless representation}. More detail on this syntax is given in section
...
\citet{pitts_locally_2023}. While this generalisation did not focus on the $\lambda$-calculus, it
proves that the syntax is able to be used effectively in Agda. This work is central in motivating an
Agda development of a formal langauge like System F using locally nameless representation, as it
helped develop the crucial initial proofs of the syntax and informed future proof strategies.

I present the first formalisation of System F using the locally nameless representation in Agda that
I am aware of. Specifically, I successfully create a formalisation which proves the following
properties of System F using locally nameless representation:
\begin{itemize}
...
\end{itemize}
An evaluaton of my formalisation as well as future work will be presented in chapter \ref{chapter4}.
...
A brief overview of Agda is presented in appendix \ref{appendix:agda_overview}. An in-depth guide to
Agda is presented in \citet{wadler_programming_2022}. Knowledge of Agda (or proof-assistants which
are similar) will be assumed henceforth.

We will make use of the Agda standard library \citep{the_agda_community_agda_2024}. The specific
imports are omitted, but are included in the full source code (see appendix
...
inductive hypothesis immediately gives the result, so the proof is left in appendix
...
\begin{code}
  ...
  subst-ty {Γ} {Δ} {X} {B = ∀ᵀ B} {C = C} lc-C C⊆Δ okΓ+map (⊢Λ И⟨ Иe₁ , Иe₂ ⟩) =
...
restrictions on the `Ok' predicate. As mentioned above, System F$_\omega$ has previously been
formalised by \citet{hutton_system_2019} in Agda, and a locally nameless approach has been used for
System F$_\omega^\ast$ by \citet{rendel_typed_2009}, an extension of System F$_\omega$ which allows
typed self-representation. Thus, an Agda development would not be unreasonable to develop.
...
\begin{code}
  ...
  subst-open-ty-ty-ty-ty {x = x} {j = j} (frᵀ z) x≢y j≥i i≻C with x ≟lchar z
  ... | yes refl = sym (lemma2·7-2-ty j≥i i≻C)
  ... | no  x≢z  = refl
  subst-open-ty-ty-ty-ty {x = x} {y = y} {j = j} (#ᵀ k) x≢y j≥i i≻C with j ≟ℕ k
  ... | no  j≢k  = refl
  ... | yes refl with x ≟lchar y
  ...   | yes refl = contradiction refl x≢y
  ...   | no  _    = refl
...
\begin{code}
  ...
  subst-intro-ty-ty {i = i} (∀ᵀ A) x∉ = cong ∀ᵀ_ (subst-intro-ty-ty {i = suc i} A x∉)
