...
In System F, types become part of the syntax, as they no become part of how terms are built. In
previous Agda formalisations, these pre-types are called `kind's \citep{hutton_system_2019}. We
follow \citet[section~5.4]{chargueraud_locally_2012} for the implementation of and implement the
syntax of kinds and terms like so.
\begin{code}
  data Kind : Set where
    k-free_  : List Char → Kind   -- Free type variables.
    k-bound_ : ℕ → Kind           -- Bound type variables.
    _k-⇒_      : Kind → Kind → Kind -- Arrow types.
    k-∀      : Kind → Kind → Kind -- "For all" type.
...
\section{Opening} In System F, we only require the opening operation
\citep{chargueraud_locally_2012}. There are three kinds of opening:
\begin{itemize}
  \item opening a kind at an index by replacing the bound type variable at that index with a free type variable,
  \item opening a term at an index by replacing the bound type variable at that index in the term with a free type variable, and
  \item opening a term at an index by replacing the bound variable at that index with a free variable.
\end{itemize}
In the
STLC, we only need to define the one opening operation because there aren't any types in the syntax.
\begin{code}
  k-k[_—→_]_ : ℕ → List Char → Kind → Kind
  k-k[ i —→ x ] (k-free y) = k-free y
  k-k[ i —→ x ] (k-bound n) with i ≟ℕ n
  ... | yes _ = k-free x
  ... | no  _ = k-bound n
  k-k[ i —→ x ] (J k-⇒ K) = (k-k[ i —→ x ] J) k-⇒ (k-k[ i —→ x ] K)
  k-k[ i —→ x ] k-∀ J K = k-∀ (k-k[ i —→ x ] J) (k-k[ suc i —→ x ] K)

  k-t[_—→_]_ : ℕ → List Char → Term → Term
  k-t[ i —→ x ] (free y) = free y
  k-t[ i —→ x ] (bound n) = bound n
  k-t[ i —→ x ] (ƛ K , L) = ƛ (k-k[ i —→ x ] K) , (k-t[ i —→ x ] L)
  k-t[ i —→ x ] (L · M) = (k-t[ i —→ x ] L) · (k-t[ i —→ x ] M)
  k-t[ i —→ x ] ‵zero = ‵zero
  k-t[ i —→ x ] (‵suc L) = ‵suc k-t[ i —→ x ] L

  t-t[_—→_] : ℕ → List Char → Term → Term
  t-t[ i —→ x ] (free y) = free y
  t-t[ i —→ x ] (bound n) with i ≟ℕ n
  ... | yes _ = free x
  ... | no  _ = bound n
  t-t[ i —→ x ] (ƛ K , L) = ƛ K , t-t[ i —→ x ] L
  t-t[ i —→ x ] (L · M) = (t-t[ i —→ x ] L) · (t-t[ i —→ x ] M)
  t-t[ i —→ x ] ‵zero = ‵zero
  t-t[ i —→ x ] (‵suc L) = ‵suc t-t[ i —→ x ] L
\end{code}
...
\begin{code}
  k-k[_←—_]_ : ℕ → List Char → Kind → Kind
  k-k[ i ←— x ] (k-free y) with x ≟lchar y
  ... | yes _ = k-bound i
  ... | no  _ = k-free y
  k-k[ i ←— x ] (k-bound n) = k-bound n
  k-k[ i ←— x ] (J k-⇒ K) = (k-k[ i ←— x ] J) k-⇒ (k-k[ i ←— x ] K)
  k-k[ i ←— x ] k-∀ J K = k-∀ (k-k[ i ←— x ] J) (k-k[ suc i ←— x ] K)
  ...
  t-t[_←—_] : ℕ → List Char → Term → Term
  t-t[ i ←— x ] (free y) with x ≟lchar y
  ... | yes _ = bound i
  ... | no  _ = free y
  t-t[ i ←— x ] (bound n) = bound n
  t-t[ i ←— x ] (ƛ K , L) = ƛ K , t-t[ i ←— x ] L
  t-t[ i ←— x ] (L · M) = (t-t[ i ←— x ] L) · (t-t[ i ←— x ] M)
  t-t[ i ←— x ] ‵zero = ‵zero
  t-t[ i ←— x ] (‵suc L) = ‵suc t-t[ i ←— x ] L
...
\begin{code}
  _≻k_ : ℕ → Kind → Set
  i ≻k K = (j : ℕ) ⦃ _ : j ≥ i ⦄ → И a , (k-k[ j —→ a ] K ≡ K)

  _≻t_ : ℕ → Term → Set
  i ≻t L = (j : ℕ) ⦃ _ : j ≥ i ⦄ → И a , (t-t[ j —→ a ] L ≡ L)

  K-LocallyClosed : Kind → Set
  K-LocallyClosed K = 0 ≻k K

  T-LocallyClosed : Term → Set
  T-LocallyClosed T = 0 ≻t T
\end{code}

[TODO: Use Pitts' definition of local closure to somehow abstract over these two similar local
closure predicates?]
