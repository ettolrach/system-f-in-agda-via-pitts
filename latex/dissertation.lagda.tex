...
\begin{code}
  ...
  subst-ty {X = Y} {L = ƛ L} lc-C okΓ+map (⊢ƛ lc-A И⟨ Иe₁ , Иe₂ ⟩) =
    ⊢ƛ (:=-≻ z≤n lc-A lc-C) И⟨ Иe₁ , (λ a {a∉} →
      let ⊢tm-tm[0→]L = Иe₂ a {a∉}
          ok = ⊢⇒Ok ⊢tm-tm[0→]L
      in subst-open-ty-tm-tm-tm-ctx L z≤n lc-C
        (subst-ty lc-C (ok-∷fv okΓ+map (:=-≻ z≤n lc-A lc-C) {!!}) ⊢tm-tm[0→]L)) ⟩
...
--   subst-ty lc-A (⊢suc ⊢L) = ⊢suc (subst-ty lc-A ⊢L)
  ...
  subst-op-ty {L} {B} {C} lc-C (⊢Λ И⟨ Иe₁ , Иe₂ ⟩) =
    ...
    in ≡-with-⊢-ty (≡-with-⊢-tm (subst-ty {Δ = ∅} lc-C ok-∅ (Иe₂ x {x∉Иe₁})) (sym (subst-intro-ty-tm L x∉ftv-L))) (sym (subst-intro-ty-ty B x∉ftv-B))
\end{code}
