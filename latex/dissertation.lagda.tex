...
module chapter4 where
  open import cofinite
  open import plfa_adaptions using (∉∷[]⇒≢; ∉-++; ∉y∷ys⇒≢y; m+1≤n⇒m≤n; All-++; ++-All)
  open import tspl_prior_work
...
\begin{code}
  ...
  open-rec-lc-lemma-ty-tm {ƛ A , L} i≢j assump rewrite
      open-rec-lc-lemma-ty {A} i≢j (proj₁ (ƛ-inj assump))
    | open-rec-lc-lemma-ty-tm {L} i≢j (proj₂ (ƛ-inj assump)) = refl
  ...
  open-rec-lc-lemma-ty-tm-tm-tm {ƛ A , L} assump rewrite
      proj₁ (ƛ-inj assump)
    | open-rec-lc-lemma-ty-tm-tm-tm {L} (proj₂ (ƛ-inj assump))
    = refl
  ...
  module example where
    ...
    twice = ⊢Λ И⟨ [] , (λ X {X∉} → ⊢ƛ fr⇒fr-lc И⟨ [] , (λ f {f∉} →
      ⊢ƛ ty-fr-lc И⟨ (f ∷ []) , (λ x {x∉} → ⊢·
        (⊢free ok-ctx (T (f≢x x∉) H′))
        (⊢· (⊢free ok-ctx (T (f≢x x∉) H′)) (⊢free ok-ctx H′))) ⟩) ⟩) ⟩
      where
        f≢x : ∀ {f x} → x ∉ f ∷ [] → f ≢ x
        f≢x x∉ = sym-≢ (∉∷[]⇒≢ x∉)
    ...
    big-omega : ∅ ⊢ (ƛ (t-∀ ((t-# 0) ⇒ (t-# 0)))
      , (((# 0) [ t-∀ ((t-# 0) ⇒ (t-# 0)) ]) · (# 0)))
        ⦂ (t-∀ ((t-# 0) ⇒ (t-# 0)))
          ⇒ (t-∀ ((t-# 0) ⇒ (t-# 0)))
    big-omega =
      ⊢ƛ lc-forall И⟨ [] , (λ x {x∉} → ⊢· (⊢[] lc-forall (⊢free ok-ctx H′)) (⊢free ok-ctx H′)) ⟩
        where
...
\begin{code}
  ...
  lemma : ∀ {Γ x A} → Ok Γ → Γ ∋ x ⦂ A → Ty-LocallyClosed A
  lemma {Γ} {x} {‵ℕ} okΓ ∋x = n≻‵ℕ
  lemma {Γ} {x} {t-fr x₁} okΓ ∋x = ty-fr-lc
  lemma {Γ} {x} {t-# k} (ok-∷fv okΓ lc-#k _) (H refl) = contradiction lc-#k #-never-lc 
  lemma {Γ} {x} {t-# k} (ok-∷fv okΓ' lc-A A⊆Γ') (T x≢y ∋x) = lemma okΓ' ∋x 
  lemma {Γ} {x} {t-# k} (ok-∷ftv (ok-∷fv okΓ' lc-#k A⊆Γ')) (T⦂ (H refl)) = contradiction lc-#k #-never-lc 
  lemma {Γ} {x} {t-# k} (ok-∷ftv (ok-∷fv okΓ' lc-A A⊆Γ')) (T⦂ (T x₁ ∋x)) = lemma okΓ' ∋x
  lemma {Γ} {x} {t-# k} (ok-∷ftv (ok-∷ftv okΓ)) (T⦂ (T⦂ ∋x)) = lemma okΓ ∋x
  lemma {Γ} {x} {A ⇒ B} okΓ ∋x = {!!}
  lemma {Γ} {x} {t-∀ A} okΓ ∋x = {!!}

  ⊢free⇒ty-lc : ∀ {Γ x A} → Γ ⊢ (fr x) ⦂ A → Ty-LocallyClosed A
  ⊢free⇒ty-lc {∅} {x} {A} (⊢free ok∅ ())
  ⊢free⇒ty-lc {Γ , y ⦂ B} {x} {A} (⊢free okΓ ∋x) = {!!}
  ⊢free⇒ty-lc {Γ , x₁} {x} {A} ⊢fr = {!!}
  ...
  rename : ∀ {Γ Δ}
    ...
    → (∀ {X} → Γ ∋ X → Δ ∋ X)
      ---------------------------------
    → (∀ {L A} → Γ ⊢ L ⦂ A → Δ ⊢ L ⦂ A)
  rename okΔ ρ-tm ρ-ty (⊢free okΓ x) = ⊢free okΔ (ρ-tm x)
...
\begin{code}
  ...
  ... | yes refl = weaken {!!} ⊢N
  ... | no  y≢y  = contradiction refl y≢y
  subst {x = y} ⊢N (⊢free {x = x} okΓ (T x≢y ∋x)) with y ≟lchar x
  ... | yes refl = contradiction refl x≢y
  ... | no  _    = ⊢free {!!} ∋x
  ...
  subst {x = y} ⊢N (⊢[] lc ⊢L) = ⊢[] lc (subst ⊢N ⊢L)
  subst {x = y} ⊢N (⊢zero okΓ) = ⊢zero {!!}
