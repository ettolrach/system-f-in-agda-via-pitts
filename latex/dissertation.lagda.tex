...
\begin{code}
  ...
  subst-ty {X = Y} lc-A (⊢free okΓ,Y lc-B ∋x) = {!!}
  subst-ty lc-A (⊢ƛ lc-B cof) = {!!}
  subst-ty lc-A (⊢· ⊢L ⊢M) = {!!}
  subst-ty lc-A (⊢Λ cof) = {!!}
  subst-ty lc-A (⊢[] lc-B ⊢L) = {!!}
  subst-ty lc-A (⊢zero okΓ) = {!!}
  subst-ty lc-A (⊢suc ⊢L) = {!!}
  ...
  subst-op-ty : ∀ {Γ L A B}
    ...
    → Γ ⊢ Λ L ⦂ t-∀ B
      --------------------
    → Γ ⊢ ty-tm[ 0 :→ A ] L ⦂ ty-ty[ 0 :→ A ] B
  subst-op-ty {Γ} {L} {A} lc-A (⊢Λ И⟨ Иe₁ , Иe₂ ⟩) =
    let x                  = fresh (fv-tm L ++ Иe₁)
        ⟨ x∉fv-L , x∉Иe₁ ⟩ = ∉-++ {xs = fv-tm L}
                                (fresh-correct (fv-tm L ++ Иe₁))
    in {!!}
...
\begin{code}
  ...
  preserve (⊢[] x ⊢L) (β-Λ 1≻L lc-A) = subst-op-ty x ⊢L
